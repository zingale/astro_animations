...
def doit():
    ...
    P_orbital = np.sqrt(4*np.pi*np.pi*a**3/(G*M_E))
    ...
    vx_init = np.sqrt( (G*M_E/a) * (1 + e) / (1 - e))
    vy_init = 0.0
    ...
    for n in range(orbit.npts):
        ...
        # plot the Earth
        ax.scatter([0], [0], s=650, color="k")
        ax.scatter([0], [0], s=600, color="b")

        # plot the orbit
        ax.plot(orbit.x[0:orbit.npts], orbit.y[0:orbit.npts],
                color="0.5", ls=":", lw=2)
        ...
        ax.fill(x_surface, y_surface, "0.75",
                edgecolor="0.75", alpha=1.0, zorder=1000)

        # plot a point on the moon's surface
        xpt = orbit.x[n] + r*np.cos(omega*orbit.t[n]+np.pi/2.0)
        ypt = orbit.y[n] + r*np.sin(omega*orbit.t[n]+np.pi/2.0)
