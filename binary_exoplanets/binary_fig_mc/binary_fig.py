...
def radial_velocity():
    ...
    #pylab.text(ss.x_star1[n]*0.75,ss.y_star1[n], "B")



    # plot star 2's orbit and position
    #symsize = 200*(M_star2/M_star1)
    pylab.plot(ss.x_star2[0:ss.npts],ss.y_star2[0:ss.npts], color="g", linestyle="--")

    pylab.scatter([ss.x_star2[n]],[ss.y_star2[n]],s=symsize,color="g", marker='h')
    ...
    xc = 0.5*(ss.x_star2[0] + ss.x_star2[0.25*ss.npts])
    yc = 0.5*(ss.y_star2[0] + ss.y_star2[0.25*ss.npts])
    
    # compute the angle of f1 wrt the initial position (just for debugging)
    a1 = math.atan2(ss.y_star2[f1*ss.npts] - yc, ss.x_star2[f1*ss.npts] - xc)
    a0 = math.atan2(ss.y_star2[0] - yc, ss.x_star2[0] - xc)

    print (a1-a0)*180./math.pi

    pylab.scatter([ss.x_star2[f1*ss.npts]],[ss.y_star2[f1*ss.npts]],s=symsize,color="g", marker='h')
    pylab.scatter([ss.x_star2[f2*ss.npts]],[ss.y_star2[f2*ss.npts]],s=symsize,color="g", marker='h')
    pylab.scatter([ss.x_star2[f3*ss.npts]],[ss.y_star2[f3*ss.npts]],s=symsize,color="g", marker='h')

    # label the points
    pylab.text(ss.x_star2[n]*1.15,ss.y_star2[n], "A4")
    pylab.text(ss.x_star2[f1*ss.npts]*1.15,ss.y_star2[f1*ss.npts]*1.15, "A1")
    pylab.text(ss.x_star2[f2*ss.npts]*1.15,ss.y_star2[f2*ss.npts], "A2")
    pylab.text(ss.x_star2[f3*ss.npts]*0.85,ss.y_star2[f3*ss.npts]*1.15, "A3")


    pylab.axis([-1.4*a_star2,0.8*a_star2,-1.4*a_star2,0.8*a_star2])

    f = pylab.gcf()
    f.set_size_inches(6.0,6.0)
  
    outfile = "binary_fig.eps"
