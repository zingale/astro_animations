...
def radial_velocity():
    ...
    # observer
    ss.y_star1[0:ss.npts] = ss.y_star1[0:ss.npts]*np.cos(np.radians(inc))
    ss.y_star2[0:ss.npts] = ss.y_star2[0:ss.npts]*np.cos(np.radians(inc))
    ...
    for n in range(ss.npts):
        ...
        plt.plot(x_rel[0:cut_index1],y_rel[0:cut_index1], 
                   color="0.75", linestyle="--",alpha=0.5)
        plt.plot(x_rel[cut_index2:ss.npts],y_rel[cut_index2:ss.npts], 
                   color="0.75", linestyle="--",alpha=0.5)


        plt.text(-1.5*a_star2, 1.1*a_star2, "star 1: T = %5.0f K" % (T1), 
                    color="r", fontsize=10)
        plt.text(-1.5*a_star2, 0.95*a_star2, "star 2: T = %5.0f K" % (T2), 
                    color="b", fontsize=10)
        plt.text(-1.5*a_star2, 0.8*a_star2, 
                    "R (star 1) / R (star 2) = %4.1f" % (R1/R2), 
                    color="k", fontsize=10)

        plt.axis([-1.5*a_star2,1.5*a_star2,-1.25*a_star2,1.25*a_star2])


        plt.subplot(212)

        plt.plot(ss.t[0:ss.npts]/tmax, f_system, "k")
        plt.scatter([ss.t[n]/tmax], [f_system[n]], color="k")
        ...
        f = plt.gcf()
        f.set_size_inches(6.0,7.2)
  
        outfile = "eclipsingbinar_{:04d}.png".format(iframe)
        plt.savefig(outfile)
