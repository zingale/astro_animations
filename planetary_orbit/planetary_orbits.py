...
def radial_velocity():
    ...
    while (n < ss.npts):
        ...
        #pylab.text(0.5*AU, -1.4*a_star2, "1 AU", horizontalalignment='center')

        #pylab.text(-1.4*a_star2, -1.3*a_star2, "time = %6.3f yr" % (ss.t[n]/year))

        pylab.scatter([0],[0],s=150,marker="x",color="k",zorder=1000)

        #pylab.text(-1.4*a_star2, 1.3*a_star2, r"mass ratio: %3.2f" % (ss.M_star1/ss.M_star2), color="k")
        #pylab.text(-1.4*a_star2, 1.1*a_star2, r"eccentricity: %3.2f" % (ecc), color="k")
        ...
        outfile = "planetary_orbit_%04d.png" % iframe
