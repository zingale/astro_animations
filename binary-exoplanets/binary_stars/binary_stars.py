...
def radial_velocity():
    ...
    while (n < ss.npts):
        ...
        symsize = 200
        if (not (M_star1 == M_star2 and ecc == 0.0)):
            pylab.plot(ss.x_star1[0:ss.npts],ss.y_star1[0:ss.npts], color="r")
        else:
            pylab.plot(ss.x_star1[0:ss.npts],ss.y_star1[0:ss.npts], color="k")

        pylab.scatter([ss.x_star1[n]],[ss.y_star1[n]],s=symsize,color="r")
        ...
        pylab.scatter([ss.x_star2[n]],[ss.y_star2[n]],s=symsize,color="g")


        if (annotate):
            # plot a reference line
            pylab.plot([0,1*AU],[-1.2*a_star2,-1.2*a_star2], color="k")
            pylab.text(0.5*AU, -1.4*a_star2, "1 AU", 
                       horizontalalignment='center')

            # display time
            pylab.text(-1.4*a_star2, -1.3*a_star2, 
                        "time = %6.3f yr" % (ss.t[n]/year))

        # display information about stars
        pylab.text(-1.4*a_star2, 1.3*a_star2, 
                    r"mass ratio: %3.2f" % (ss.M_star1/ss.M_star2), color="k")
        pylab.text(-1.4*a_star2, 1.1*a_star2, 
                    r"eccentricity: %3.2f" % (ecc), color="k")


        # energies
        if (annotate):
            KE1 = 0.5*ss.M_star1*(ss.vx_star1[n]**2 + ss.vy_star1[n]**2)
            KE2 = 0.5*ss.M_star2*(ss.vx_star2[n]**2 + ss.vy_star2[n]**2)
            PE = -G*ss.M_star1*ss.M_star2/ \
                math.sqrt((ss.x_star1[n] - ss.x_star2[n])**2 + \
                          (ss.y_star1[n] - ss.y_star2[n])**2)


            print KE1, KE2, PE, KE1 + KE2 + PE
            # KE 1
            a, b = find_scinotat(KE1)
            pylab.text(0, 1.3*a_star2, r"$K_1 =$")
            pylab.text(0.3*a_star2, 1.3*a_star2, r"$%+4.2f \times 10^{%2d}$ erg" % (a,b))

            a, b = find_scinotat(KE2)
            pylab.text(0, 1.15*a_star2, r"$K_2 =$")
            pylab.text(0.3*a_star2, 1.15*a_star2, r"$%+4.2f \times 10^{%2d}$ erg" % (a,b))

            a, b = find_scinotat(PE)            
            pylab.text(0, 1.0*a_star2, r"$U =$")
            pylab.text(0.3*a_star2, 1.0*a_star2, r"$%+4.2f \times 10^{%2d}$ erg" % (a,b))

            a, b = find_scinotat(KE1 + KE2 + PE)            
            pylab.text(0, 0.85*a_star2, r"$E =$")
            pylab.text(0.3*a_star2, 0.85*a_star2, r"$%+4.2f \times 10^{%2d}$ erg" % (a,b))

        pylab.axis([-1.4*a_star2,1.4*a_star2,-1.4*a_star2,1.4*a_star2])
